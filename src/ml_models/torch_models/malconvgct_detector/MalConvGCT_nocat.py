...
from adversarial_malware_samples_generation.ml_models.torch_models.malconvgct_detector.LowMemConv import LowMemConvBase
from adversarial_malware_samples_generation.ml_models.torch_models.malconvgct_detector.MalConvML import MalConvML
...
class MalConvGCT(LowMemConvBase):
    ...
    def __init__(self, out_size=2, channels=128, window_size=512, stride=512, layers=1, embd_size=8, log_stride=None, low_mem=True, thresh=0.5):
    ...
    def predict(self, bytez: np.ndarray):
        np_bytez = np.frombuffer(bytez, dtype=np.uint8)[np.newaxis,:]
        _inp = torch.from_numpy(np_bytez.copy())
        #_inp = torch.from_numpy(x[np.newaxis, :])
        x, penult, post_conv = self.custom_forward(_inp)
        outputs = F.softmax(x, dim=-1)
        return float(outputs[0][1]), outputs.detach().numpy()[0,1] > self.thresh



    def get_predicted_from_outputs(self, outputs):
        """
        _, predicted = torch.max(outputs.data, 1)
        return predicted
        """
        probs = F.softmax(outputs, dim=-1)
        predicted = torch.IntTensor([1 if prob[1] > self.thresh else 0 for prob in probs])
        return predicted

    def predict_from_bytez(self, bytez):
