...
import argparse
sys.path.append("../../../../")
from src.gan_implementations.malgan.byte.malgan_v3 import SkeletonMalGAN
...
class MalGAN(SkeletonMalGAN):
    ...
    def predict_labels_with_blackbox_model(self, features:dict):
        if self.sorel20m_ffnn_model is not None:
            if type(features["ember_v2"]) == np.array or type(features["ember_v2"]) == np.ndarray:
                ember_features = torch.from_numpy(features["ember_v2"])
            else:
                ember_features = features["ember_v2"]

            model_output = self.sorel20m_ffnn_model(ember_features)["malware"]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_filepath",
                        ...
                        default="models/MalGAN_sorel20m_ffnn/")
