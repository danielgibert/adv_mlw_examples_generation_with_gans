...
class ModularEmberFeatureExtractor(object):
    ...
    def __init__(
            ...
            byte_entropy_histogram=False,
            string_features=False,
            general_file_features=False,
            header_file_features=False,
            section_features=False,
            imports_features=False,
            exports_features=False,
            data_features=False
    ):
        self.features_version = feature_version
        self.features = []
        if byte_histogram is True:
            self.features.append(ByteHistogram()) # 256
        if byte_entropy_histogram is True:
            self.features.append(ByteEntropyHistogram()) # 256
        if string_features is True:
            self.features.append(StringExtractor()) # 1 + 1 + 1 + 96 + 1 + 1 + 1 + 1 + 1
        if general_file_features is True:
            self.features.append(GeneralFileInfo()) # 10
        if header_file_features is True:
            self.features.append(HeaderFileInfo()) # 62
        if section_features is True:
            self.features.append(SectionInfo()) # 5 + 50 + 50 + 50 + 50 + 50
        if imports_features is True:
            self.features.append(ImportsInfo()) # 1280
        if exports_features is True:
            self.features.append(ExportsInfo()) # 128
        ...
        if feature_version == 1:
            if not lief.__version__.startswith("0.8.3"):
                if print_feature_warning:
                    ...
                    print("WARNING:   lief version {} found instead. There may be slight inconsistencies".format(lief.__version__))
                    print("WARNING:   in the feature calculations.")
        elif feature_version == 2:
            if data_features is True:
                self.features.append(DataDirectories()) # 15 * 2
            ...
            raise Exception("EMBER feature version must be 1 or 2. Not {}".format(feature_version))
    ...
    def replace_hashed_import_features(self, ember_features, hashed_import_features):
        # It starts at position 943: 943+1280
        # Need to replace the corresponding feature subset by the new hashed import features
        start_position = 943
        end_position = 943 + 1280
        return np.concatenate((ember_features[:start_position], hashed_import_features, ember_features[end_position:]))

    def replace_byte_histogram_features(self, ember_features, byte_histogram_features):
        end_position = 256
        return np.concatenate((byte_histogram_features, ember_features[end_position:]))

    def replace_hashed_strings_features(self, ember_features, string_features):
        start_position = 256 + 256 # ByteHistogram + ByteEntropyHistogram
        end_position = 256 + 256 + 1 + 1 + 1 + 96 + 1 + 1 + 1 + 1 + 1
        return np.concatenate((ember_features[:start_position], string_features, ember_features[end_position:]))



if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='EMBER Feature Extractor')
    parser.add_argument("exe_filepath",
                        type=str,
                        help="Filepath of the executable")
    parser.add_argument("--version",
                        type=int,
                        help="Features Version",
                        default=2)
    parser.add_argument("--output_filepath",
                        type=str,
                        help="Output filepath (pickle format)",
                        default=None)
    args = parser.parse_args()

    feature_extractor = ModularEmberFeatureExtractor(feature_version=args.version)
    with open(args.exe_filepath, "rb") as input_file:
        bytez = input_file.read()
    ember_features = feature_extractor.feature_vector(bytez)

    if args.output_filepath is not None:
        #savez_compressed(os.path.join(imports_features_filepath, '{}.npz'.format(pe_modifier.sha256)), imports_array)
        savez_compressed(args.output_filepath, ember_features)
        #ember_features = np.load(args.output_filepath, allow_pickle=True)["arr_0"]

    print("Features: ", ember_features)
    print("Shape: ", ember_features.shape)
